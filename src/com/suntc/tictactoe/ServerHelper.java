...
public class ServerHelper implements Runnable
{
	public static synchronized void CheckWaiting()
	{
		if (Server.WaitingArray.size() >= 2)
		{
			...
			Game game = new Game();
			PlayerHelper player1 = new PlayerHelper(game, Server.WaitingArray.get(0), Server.ProConArray.get(0));//,Server.LockArray.get(0),Server.CondArray.get(0),Server.InGameBoolArray.get(0));
			PlayerHelper player2 = new PlayerHelper(game, Server.WaitingArray.get(1), Server.ProConArray.get(1));//,Server.LockArray.get(1),Server.CondArray.get(1),Server.InGameBoolArray.get(1));
